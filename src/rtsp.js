...
class RTSPManager extends EventEmitter {
  constructor(options = {}) {
    ...
    this.mode         = 'idle';   // 'idle' | 'video' | 'visualization' | 'karaoke'
    this.enabled      = false;
    this.displayMode  = 'video';  // 'video' | 'karaoke'
    ...
    this.lyricFile    = path.join(os.tmpdir(), 'streamy-lyric.txt');
  }
  ...
  async streamTrack(track) {
    ...
    this._writeLyricLine('');   // clear lyric on new track

    await new Promise(r => setTimeout(r, 600));

    if (this.displayMode === 'karaoke') {
      this._startKaraokeMode(track);
    } else {
  ...
  /** Set display mode: 'video' or 'karaoke' */
  setDisplayMode(mode) {
    this.displayMode = mode === 'karaoke' ? 'karaoke' : 'video';
    // Restart current stream in new mode
    if (this.currentTrack && this.enabled) {
      this.streamTrack(this.currentTrack);
    }
  }

  /** Update the current lyric line shown in karaoke mode */
  updateLyricLine(line) {
    this._writeLyricLine(line || '');
  }
...
`;
  ...
  _startKaraokeMode(track) {
    this._writeOverlay(track);
    this._writeLyricLine('');
    const rtspUrl = `rtsp://localhost:${this.rtspPort}/live`;
    const w = this.width, h = this.height;

    // Deep purple/black animated background
    const bgSrc = [
      `nullsrc=size=${w}x${h}:rate=30`,
      `geq=r='20+15*sin(X/80+t*0.4)':g='10+10*sin(Y/70+t*0.3+1)':b='40+30*sin((X+Y)/90+t*0.5+2)'`,
      'format=yuv420p',
    ].join(',');

    const lf  = this.lyricFile.replace(/\\/g, '/');
    const tf  = this.titleFile.replace(/\\/g, '/');
    const af  = this.artistFile.replace(/\\/g, '/');

    // Lyric line — large, centred, with a subtle glow shadow
    const lyricVF = [
      // Dark gradient bar behind lyric text
      `drawbox=x=0:y=${h/2 - 80}:w=iw:h=160:color=black@0.6:t=fill`,
      // Main lyric line — big and centred
      `drawtext=textfile='${lf}':reload=1:fontsize=72:fontcolor=white:` +
        `x=(w-tw)/2:y=(h-th)/2:shadowcolor=0x7c5af0@0.9:shadowx=0:shadowy=0:` +
        `borderw=3:bordercolor=0x7c5af0@0.6`,
      // Track info bar at bottom
      `drawbox=x=0:y=${h - 90}:w=iw:h=90:color=black@0.7:t=fill`,
      `drawtext=textfile='${tf}':reload=1:fontsize=34:fontcolor=white:` +
        `x=(w-tw)/2:y=${h - 78}:shadowcolor=black@0.9:shadowx=2:shadowy=2`,
      `drawtext=textfile='${af}':reload=1:fontsize=24:fontcolor=white@0.7:` +
        `x=(w-tw)/2:y=${h - 42}:shadowcolor=black@0.9:shadowx=1:shadowy=1`,
    ].join(',');
    ...
    this._launchFFmpeg(args, track, false);
    this.mode = 'karaoke';
    this.emit('streamStart', { mode: 'karaoke', track });
    console.log(`[RTSP] 🎤 Karaoke mode: ${track.title}`);
  }

  _writeLyricLine(line) {
    try {
      // FFmpeg drawtext can struggle with special chars — sanitise
      const safe = String(line || '')
        ...
        .trim()
        .slice(0, 80);
      fs.writeFileSync(this.lyricFile, safe);
    } catch {}
  }
