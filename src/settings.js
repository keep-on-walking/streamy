...
function testOAuth2() {
  return new Promise(resolve => {
    ...
    const proc = spawn('yt-dlp', [
      ...
      '--no-warnings', '--skip-download', '--print', '%(channel)s',
      'https://www.youtube.com/watch?v=dQw4w9WgXcQ',
    ...
    proc.on('close', code => {
      const name = out.trim();
      if (code === 0 && name && name !== 'NA') {
        resolve({ success: true, accountName: name });
      } else if (err.includes('Sign in') || err.includes('login') || code !== 0) {
