...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    // Save references to previous processes for seamless overlap
    const prevMpv  = this.mpvProc;
    const prevIdle = this.idleProc;
    this.mpvProc  = null;
    this.idleProc = null;
    this._disconnectIPC();
    try { fs.unlinkSync(this.socketPath); } catch {}
    ...
    this.mpvProc.on('exit', (code, signal) => {
      ...
      if (stderrBuf.includes('ERROR') || code !== 0) {
        console.error('[mpv] playback error, code:', code);
        this.emit('trackError', track);
      } else {
        this.emit('trackEnd');
      }
    });
    ...
    // Wait for new mpv IPC socket (window is up), then kill previous processes
    const ready = await this._waitForSocket(5000);

    // Now seamlessly swap — kill old mpv/idle after new window is rendering
    if (prevMpv) {
      prevMpv.removeAllListeners('exit');
      try { prevMpv.kill('SIGTERM'); } catch {}
      setTimeout(() => { try { prevMpv.kill('SIGKILL'); } catch {} }, 2000);
    }
    if (prevIdle) {
      try { prevIdle.kill('SIGTERM'); } catch {}
    }
  ...
  _sendIPC(...args) {
    if (!this.ipcSocket || !this.ipcReady) return;
