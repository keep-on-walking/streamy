...
function safeUrl(str = '') {
  // Keep URL intact but escape only chars that break ffmpeg drawtext: ' and   return String(str).replace(/\\/g, '\\\\').replace(/'/g, "\\'").slice(0, 80);
}
...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    if (this.displayMode === 'visualization' && this.mpvVO !== 'drm') {
      ...
      // PC: exclude AV1 (no hwdec on OptiPlex 9020M) and cap at maxHeight (default 1080p)
  ...
  _handleIPCMessage(msg) {
    if (msg.event === 'property-change') {
      ...
      // Show loading screen immediately to prevent desktop flash while
      // server.js handles the trackEnd event and queues the next track
  ...
  async _generateIdleScreen() {
    ...
    try {
      ...
      // Logo: 'streamy' wordmark + 'JUKEBOX' tagline
      // QR version: logo at top (y=100), QR centred, scan text below
      // No-QR version: logo centred, URL below

      if (hasQR) {
        filters.push(
          `[1:v]scale=260:260[qr]`,
          `[0:v][qr]overlay=(W-w)/2:(H-h)/2+30[bg]`,
          `[bg]drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=(w-tw)/2:y=80,` +
          `drawtext=text='JUKEBOX':fontsize=30:fontcolor=0x8b82cc:x=(w-tw)/2+46:y=190,` +
          `drawtext=text='Scan to add music':fontsize=26:fontcolor=white@0.7:x=(w-tw)/2:y=h-130,` +
          `drawtext=text='${safeUrl(remoteUrl)}':fontsize=18:fontcolor=white@0.35:x=(w-tw)/2:y=h-90`
        );
      } else {
        filters.push(
          `[0:v]drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=(w-tw)/2:y=(h/2)-100,` +
          `drawtext=text='JUKEBOX':fontsize=30:fontcolor=0x8b82cc:x=(w-tw)/2+46:y=(h/2)+10,` +
          `drawtext=text='${safeUrl(remoteUrl)}':fontsize=24:fontcolor=white@0.6:x=(w-tw)/2:y=(h/2)+80,` +
          `drawtext=text='Open this URL to add music':fontsize=20:fontcolor=white@0.35:x=(w-tw)/2:y=(h/2)+120`
        );
      }
      ...
      try {
        await this._run(`ffmpeg -y -f lavfi -i "color=c=0x0a0a14:s=${w}x${h}:d=1" -vf "drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=(w-tw)/2:y=(h/2)-80,drawtext=text='JUKEBOX':fontsize=30:fontcolor=0x8b82cc:x=(w-tw)/2:y=(h/2)+20" -frames:v 1 "${this.idleImagePath}"`);
      } catch {}
    }
  }
  ...
  async _generateLoadingScreen() {
    const w = this.width, h = this.height;
    try {
      await this._run(`ffmpeg -y -f lavfi -i "color=c=0x0a0a14:s=${w}x${h}:d=1" -vf "drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=(w-tw)/2:y=(h/2)-100,drawtext=text='JUKEBOX':fontsize=30:fontcolor=0x8b82cc:x=(w-tw)/2+46:y=(h/2)+10,drawtext=text='Loading...':fontsize=24:fontcolor=white@0.4:x=(w-tw)/2:y=(h/2)+70" -update 1 -frames:v 1 "${this.loadingImgPath}"`);
    } catch {}
  }
  ...
  _getAudioDevice() {
    ...
    // Map friendly names to ALSA device patterns
    ...
    if (this.audioOutput === 'headphone') {
      try {
        const { execSync } = require('child_process');
        const devices = execSync('mpv --audio-device=help 2>&1', { encoding: 'utf8', timeout: 5000 });
        // Match headphone or analog stereo (the headphone jack on most PCs)
        const hp = devices.match(/\s+'(alsa\/\S*(?:[Hh]eadphone|analog)\S*)'/);
        if (hp) return hp[1];
        // Try pipewire/pulse analog
