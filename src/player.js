...
class Player extends EventEmitter {
  ...
  /**
   * Detect when vocals first appear in a video's audio stream.
   * Uses ffmpeg bandpass filter (vocal range 300-3000Hz) + silence detection.
   * Returns the timestamp in seconds where vocals likely begin, or null on failure.
   */
  detectVocalOnset(videoId) {
    return new Promise(resolve => {
      const url = `https://www.youtube.com/watch?v=${videoId}`;
      const timeout = setTimeout(() => {
        console.warn('[vocal-detect] timed out');
        resolve(null);
      }, 30000);

      // Build yt-dlp args for audio extraction
      const ytdlpArgs = [
        ...this.cookieArgs,
        '-f', 'bestaudio',
        '-o', '-',
      ...
      // ffmpeg: take first 90 seconds, bandpass for vocal frequencies, detect silence gaps
      const ffmpegArgs = [
        '-i', 'pipe:0',
        '-t', '90',
        '-af', 'bandpass=f=1500:width_type=h:w=2700,silencedetect=n=-28dB:d=0.3',
        '-f', 'null', '-',
      ];
      ...
      ffmpeg.on('close', () => {
        ...
        // Parse silence_end timestamps — first one after a silence period is likely vocal onset
        const silenceEnds = [];
        const re = /silence_end:\s*([\d.]+)/g;
        let m;
        while ((m = re.exec(stderrBuf)) !== null) {
          silenceEnds.push(parseFloat(m[1]));
        }

        if (silenceEnds.length > 0) {
          // The first silence_end is when audio first rises above threshold in vocal range
          const onset = silenceEnds[0];
          console.log(`[vocal-detect] onset at ${onset.toFixed(2)}s (${silenceEnds.length} transitions found)`);
          resolve(onset);
        } else {
          // No silence detected — vocals likely start immediately or detection failed
          console.log('[vocal-detect] no silence transitions found — vocals may start at 0');
          resolve(0);
        }
      });
