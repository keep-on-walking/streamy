...
class Player extends EventEmitter {
  ...
  async _generateIdleScreen() {
    ...
    try {
      ...
      // Logo: 'streamy' wordmark + play triangle + 'JUKEBOX' tagline
      const lx = `(w-480)/2`;
      const logo =
        `drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=${lx}:y=(h/2)-130,` +
        `drawtext=text='JUKEBOX':fontsize=30:fontcolor=0x8b82cc:x=${lx}+46:y=(h/2)-20`;

      if (hasQR) {
        filters.push(
          `[1:v]scale=240:240[qr]`,
          `[0:v][qr]overlay=(W-w)/2:(H-h)/2+70[bg]`,
          `[bg]${logo},` +
          `drawtext=text='Scan to add music':fontsize=26:fontcolor=white@0.7:x=(w-tw)/2:y=(h/2)+240,` +
          `drawtext=text='${safeText(remoteUrl)}':fontsize=18:fontcolor=white@0.35:x=(w-tw)/2:y=(h/2)+278`
        );
      } else {
        filters.push(
          `[0:v]${logo},` +
          `drawtext=text='${safeText(remoteUrl)}':fontsize=24:fontcolor=white@0.6:x=(w-tw)/2:y=(h/2)+40,` +
          `drawtext=text='Open this URL to add music':fontsize=20:fontcolor=white@0.35:x=(w-tw)/2:y=(h/2)+80`
        );
      }
