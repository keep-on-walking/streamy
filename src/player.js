...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    const args = [
      ...
      `--hwdec=${this.mpvHwdec}`,
      '--profile=fast',
      '--af=dynaudnorm=g=5:f=250:r=0.9:p=0.7',
      ...this.mpvExtra,
    ];
    ...
    // DRM mode: wait for mpv to render first frame, then unblank
    if (this.mpvVO === 'drm') {
  ...
  async _generateLoadingScreen() {
    // Loading image generated but only used as fallback — primary method is fb0 blank
    const w = this.width, h = this.height;
    try {
      await this._run(`ffmpeg -y -f lavfi -i "color=c=0x0a0a14:s=${w}x${h}:d=1" -vf "drawtext=text='Streamy':fontsize=64:fontcolor=0x7c5af0:x=(w-tw)/2:y=(h/2)-60:shadowcolor=black@0.8:shadowx=3:shadowy=3,drawtext=text='Loading next track...':fontsize=28:fontcolor=white@0.5:x=(w-tw)/2:y=(h/2)+20" -update 1 -frames:v 1 "${this.loadingImgPath}"`);
    } catch {}
  }
  ...
  _showLoadingScreen() {
    ...
    try {
      // Blank framebuffer — instant black screen
