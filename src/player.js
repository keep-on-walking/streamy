...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    // In DRM mode: make console invisible so nothing shows during mpv transition
    if (this.mpvVO === 'drm') {
      try {
        const { execSync } = require('child_process');
        // Hide cursor, clear screen, set black foreground on black background
        execSync('echo -ne "\\033[?25l\\033[2J\\033[H\\033[0;30m\\033[40m" > /dev/tty1 2>/dev/null || true', { stdio: 'ignore' });
        // Also blank VT to prevent any text from appearing
        execSync('echo -ne "\\033[9;0]" > /dev/tty1 2>/dev/null || true', { stdio: 'ignore' });
      } catch {}
    }
    ...
    if (this.displayMode === 'visualization' || this.displayMode === 'karaoke') {
      // Audio-only with waveform visualization
      args.push(
      ...
      // Video mode — best quality up to configured max
  ...
  stop() {
    if (this.mpvVO === 'drm') {
      try { require('child_process').execSync('echo -ne "\\033[?25l\\033[2J\\033[H\\033[0;30m\\033[40m" > /dev/tty1 2>/dev/null || true', { stdio: 'ignore' }); } catch {}
    }
    this._killMpv();
    this.currentTrack   = null;
    this.state.playing  = false;
