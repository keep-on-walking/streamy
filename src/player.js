...
class Player extends EventEmitter {
  ...
  async _generateIdleScreen() {
    ...
    // Build idle image using Python/Pillow for full shape + text control
...
if '${qrArg}':
    # QR layout: logo near top, QR in middle, scan text at bottom
    logo_y = 80
    wordmark_x = (w - tw) // 2
    draw.text((wordmark_x, logo_y), txt_streamy, font=font_lg, fill=(240, 238, 255))

    # Play triangle — aligned left of JUKEBOX, vertically centred on it
    ...
    try:
        ...
        print('QR paste error:', e, file=sys.stderr)
    ...
    url_txt = '${remoteUrl}'
    bb3 = draw.textbbox((0,0), url_txt, font=font_url)
    draw.text(((w - (bb3[2]-bb3[0])) // 2, h - 90), url_txt, font=font_url, fill=(255,255,255,89))

else:
    # No-QR layout: logo centred, URL below
    logo_y = h//2 - 100
    wordmark_x = (w - tw) // 2
    draw.text((wordmark_x, logo_y), txt_streamy, font=font_lg, fill=(240, 238, 255))
    ...
    url_txt = '${remoteUrl}'
    bb2 = draw.textbbox((0,0), url_txt, font=font_url)
    draw.text(((w - (bb2[2]-bb2[0])) // 2, logo_y + 180), url_txt, font=font_url, fill=(255,255,255,153))
    tip_txt = 'Open this URL to add music'
    bb3 = draw.textbbox((0,0), tip_txt, font=font_tip)
    draw.text(((w - (bb3[2]-bb3[0])) // 2, logo_y + 220), tip_txt, font=font_tip, fill=(255,255,255,89))
...
`;
    try {
      ...
      try {
        await this._run(`ffmpeg -y -f lavfi -i "color=c=0x0a0a14:s=${w}x${h}:d=1" -vf "drawtext=text='streamy':fontsize=96:fontcolor=0xf0eeff:x=(w-tw)/2:y=(h/2)-80" -frames:v 1 "${this.loadingImgPath}"`);
      } catch {}
    }
  }
