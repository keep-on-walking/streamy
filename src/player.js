...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    await new Promise(r => setTimeout(r, this.mpvVO === 'drm' ? 100 : 300));
  ...
  _getAudioDevice() {
    ...
    if (this.audioOutput === 'headphone') {
      try {
        const { execSync } = require('child_process');
        const devices = execSync('mpv --audio-device=help 2>&1', { encoding: 'utf8', timeout: 5000 });
        const hp = devices.match(/\s+(alsa\/\S*[Hh]eadphone\S*)/);
        if (hp) return hp[1];
      } catch {}
      return 'alsa/default:CARD=Headphones';
    }
    return null;
  }
      ...
      for (const line of lines) {
        ...
        if (/hdmi/i.test(device)) {
          ...
          if (!outputs.find(o => o.id === 'headphone')) {
            outputs.push({ id: 'headphone', name: `Headphone (${desc.trim()})`, active: this.audioOutput === 'headphone' });
          }
        }
      }
    } catch {}
