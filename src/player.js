...
class Player extends EventEmitter {
  ...
  async playTrack(track) {
    ...
    // In DRM mode: switch to empty VT to hide console during mpv transition
    if (this.mpvVO === 'drm') {
      try {
        const { execSync } = require('child_process');
        execSync('sudo chvt 7 2>/dev/null; echo -ne "\\033[?25l" > /dev/tty7 2>/dev/null || true', { stdio: 'ignore', timeout: 2000 });
      } catch {}
    }
  ...
  stop() {
    if (this.mpvVO === 'drm') {
      try { require('child_process').execSync('sudo chvt 7 2>/dev/null; echo -ne "\\033[?25l" > /dev/tty7 2>/dev/null || true', { stdio: 'ignore', timeout: 2000 }); } catch {}
    }
    this._killMpv();
    this.currentTrack   = null;
    this.state.playing  = false;
