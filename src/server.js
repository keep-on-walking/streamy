...
app.get('/api/stream/:videoId', (req, res) => {
  ...
  // yt-dlp fetches WebM/Opus, ffmpeg remuxes to OGG (same Opus codec, no re-encoding)
  // OGG is a streaming container — browser can parse from byte 0 without seeking
  ...
  const ff = spawn('ffmpeg', [
    ...
    '-i', 'pipe:0',
    '-vn',
    '-c:a', 'copy',   // no re-encoding — just remux Opus into OGG container
    '-f', 'ogg',
    'pipe:1',
  ]);
  ...
  req.on('close', () => { ytdlp.kill(); ff.kill(); });
