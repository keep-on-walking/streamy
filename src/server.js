...
app.get('/api/search', (req, res) => {
  ...
  ytdlp.on('close', code => {
    ...
    try {
      ...
      const KIDS_CHANNELS = /kids|toys|nursery|rhymes|toddler|cocomelon|baby shark|diana and roma|vlad and niki/i;
      const results = (raw.entries || [])
        .filter(e => e.duration && e.duration >= 30 && e.duration <= 600)
        .filter(e => !KIDS_CHANNELS.test(e.uploader || '') && !KIDS_CHANNELS.test(e.channel || ''))
        .slice(0, 10)
        .map(e => ({
          videoId:   e.id,
          title:     e.title,
