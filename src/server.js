...
const {
  ...
  fetchLikedSongs, detectBrowsers, COOKIES_PATH,
} = require('./settings');
...
app.get('/api/search', (req, res) => {
  ...
  const cookieArgs = getCookieArgs(config).join(' ');
  const cmd = `yt-dlp ${cookieArgs} "ytsearch10:${q.replace(/"/g, '\\"')}" --flat-playlist --dump-single-json --no-warnings 2>/dev/null`;
...
app.post('/api/settings/youtube/test', async (req, res) => {
  let result;
  if (config.youtube.authMethod === 'browser-cookies') result = await testBrowserCookies(config.youtube.browser);
  else if (config.youtube.authMethod === 'cookie-file') result = await testCookieFile();
  else return res.json({ success: false, error: 'No auth method configured' });

  config.youtube.loggedIn    = result.success;
  config.youtube.accountName = result.success ? result.accountName : config.youtube.accountName;
