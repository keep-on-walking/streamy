  ...
  try {
    cdnUrl = await new Promise((resolve, reject) => {
      ...
      proc.on('close', code => {
        ...
        (code === 0 && url) ? resolve(url) : reject(new Error('yt-dlp failed'));
      });
      proc.on('error', reject);
      setTimeout(() => { proc.kill(); reject(new Error('timeout')); }, 15000);
    });
  } catch (e) {
    console.error('[stream] yt-dlp error:', e.message);
    return res.status(500).json({ error: 'Could not get stream URL' });
  }

  // Step 2: proxy CDN URL via axios (handles redirects, timeouts, range requests)
  try {
    const proxyHeaders = {
    ...
    const status = upstream.status; // 200 or 206 for range
    res.status(status);
