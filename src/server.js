...
async function autoSyncLyrics(track, lrc) {
  try {
    // Parse first meaningful lyric line timestamp (skip empty/instrumental lines)
    const lines = lrc.split('\n');
    let firstLyricTime = null;
    for (const line of lines) {
      const m = line.match(/\[(\d+):(\d+\.\d+)\](.*)/);
      if (!m) continue;
      const text = m[3].trim();
      if (!text || /^\s*$/.test(text)) continue;
      firstLyricTime = parseInt(m[1]) * 60 + parseFloat(m[2]);
      break;
    }
    if (firstLyricTime === null) return;

    console.log(`[auto-sync] first lyric at ${firstLyricTime.toFixed(2)}s — detecting vocal onset...`);

    const videoOnset = await player.detectVocalOnset(track.videoId);
    if (videoOnset === null) {
      console.log('[auto-sync] detection failed — using 0 offset');
      return;
    }

    // Offset = where vocals are in the video − where lyrics say they should be
    const offset = Math.round((videoOnset - firstLyricTime) * 10) / 10;
    console.log(`[auto-sync] video vocal onset: ${videoOnset.toFixed(2)}s, lyric first line: ${firstLyricTime.toFixed(2)}s → offset: ${offset.toFixed(1)}s`);

    // Only apply if offset is significant (>0.5s) and reasonable (<30s)
    if (Math.abs(offset) > 0.5 && Math.abs(offset) < 30) {
      broadcast('lyricOffset', { offset });
      console.log(`[auto-sync] applied offset: ${offset.toFixed(1)}s`);
    } else if (Math.abs(offset) <= 0.5) {
      console.log('[auto-sync] offset small enough to ignore');
    } else {
      console.log('[auto-sync] offset too large, likely wrong — ignoring');
    }
  } catch (e) {
