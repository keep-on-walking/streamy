...
app.get('/api/search', (req, res) => {
  ...
  ytdlp.on('close', code => {
    ...
    try {
      ...
      const NON_MUSIC = /kids|toys|nursery|rhymes|toddler|cocomelon|baby shark|diana and roma|vlad and niki|peppa|pinkfong|little angel|bounce patrol|dave and ava|super simple|chu chu|badanamu|kidz bop|gracie.s corner|power kidz|play doh|unboxing|toy review|mukbang|asmr food|recipe|cooking with|how to cook|gameplay|playthrough|walkthrough|let.s play|tutorial|diy craft|politics|pacifier|podcast|react|reaction|prank|news|documentary|compilation|top \d+|best of 20|worst of|interview|behind the scenes|explained|review|trailer|teaser|clip|scene|episode|full movie|full film|audiobook|meditation|sleep sounds|white noise|rain sounds|asmr|satisfying|tiktok|shorts|challenge|try not to|funny moments|fail|bloopers|cartoon|animation|animated|storytime|reading|lecture|ted talk|how to|what is|timer|countdown|test tone|sound effect|ringtone/i;

      // Additional title-only patterns (too common in channel names to block globally)
      const NON_MUSIC_TITLE = /\b(part \d+|ep\.?\s*\d+|episode \d+|chapter \d+|day \d+|round \d+|level \d+|stage \d+|week \d+|hour loop|hours? of|10 hours|1 hour|extended version|slowed.*reverb|sped up|bass boosted|8d audio|nightcore)\b/i;
  ...
  try {
    ...
    for (const q of queries) {
      try {
        ...
        const NON_MUSIC = /kids|toys|nursery|rhymes|toddler|cocomelon|baby shark|peppa|pinkfong|little angel|kidz bop|play doh|unboxing|toy review|mukbang|asmr food|recipe|cooking|gameplay|walkthrough|tutorial|diy craft|politics|podcast|react|reaction|prank|news|documentary|compilation/i;
