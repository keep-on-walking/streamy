...
const player = new Player({
  ...
  maxHeight:    config.platform?.maxHeight   || 2160,
...
app.get('/api/search', (req, res) => {
  ...
  // Timeout: kill yt-dlp if it takes too long
  ...
  ytdlp.on('close', code => {
    ...
    try {
      ...
      // Block non-music content by channel name, title, or uploader
      // Use \b word boundaries to avoid false positives (e.g. "kids" in "Lippy Kids")
      const NON_MUSIC_CHANNEL = /\bfor kids\b|toys|nursery|rhymes|toddler|cocomelon|baby shark|diana and roma|vlad and niki|peppa|pinkfong|little angel|bounce patrol|dave and ava|super simple|chu chu|badanamu|kidz bop|gracie.s corner|power kidz|play doh|unboxing|toy review|mukbang|asmr food|gameplay|playthrough|walkthrough|let.s play|diy craft|pacifier|storytime|ted talk/i;
      const NON_MUSIC_TITLE = /\b(unboxing|toy review|mukbang|recipe|cooking with|how to cook|gameplay|playthrough|walkthrough|let.s play|tutorial|diy craft|politics|podcast|prank|documentary|compilation|top \d+ |best of 20|worst of|interview|behind the scenes|explained|movie review|trailer|teaser|full movie|full film|audiobook|meditation|sleep sounds|white noise|rain sounds|satisfying|tiktok comp|try not to|funny moments|fail comp|bloopers|cartoon|storytime|reading|lecture|ted talk|timer|countdown|test tone|sound effect|ringtone|part \d+|ep\.?\s*\d+|episode \d+|chapter \d+|day \d+|\d+ hours? loop|10 hours|1 hour|extended version|slowed.*reverb|sped up|bass boosted|8d audio|nightcore)\b/i;

      const filtered = (raw.entries || [])
        .filter(e => e.duration && e.duration >= 60 && e.duration <= 600)
        .filter(e => !NON_MUSIC_CHANNEL.test(e.uploader || '') && !NON_MUSIC_CHANNEL.test(e.channel || ''))
        .filter(e => !NON_MUSIC_TITLE.test(e.title || ''));

      // Score results: prefer verified channels, VEVO, official content, music keywords
      const MUSIC_SIGNALS = /official|vevo|music|audio|lyrics|lyric|feat\.?|ft\.?|prod\.?|remix|acoustic|live at|concert|session|unplugged|stripped|deluxe|remaster|video|visuali[sz]er|performance/i;
      const MUSIC_CHANNEL = /records|music|entertainment|vevo|warner|sony|universal|atlantic|island|capitol|republic|interscope|def jam|parlophone|polydor|columbia|rca|epic|virgin|emi/i;
      const scored = filtered.map(e => {
        let score = 0;
        if (e.channel_is_verified) score += 3;
        if (/vevo/i.test(e.channel || '') || /vevo/i.test(e.uploader || '')) score += 4;
        if (MUSIC_SIGNALS.test(e.title || '')) score += 2;
        if (MUSIC_CHANNEL.test(e.channel || '') || MUSIC_CHANNEL.test(e.uploader || '')) score += 2;
        if ((e.view_count || 0) > 10000000) score += 2;
        else if ((e.view_count || 0) > 1000000) score += 1;
        // Penalise very short titles (likely non-music)
        if ((e.title || '').length < 5) score -= 2;
...
app.get('/api/popular', async (req, res) => {
  ...
  // Curated queries that reliably return popular music tracks
  const queries = [
    'most viewed music videos this month',
    'new music friday official',
    'trending music videos official',
  ];
  ...
  try {
    ...
    for (const q of queries) {
      try {
        ...
        const NON_MUSIC_CH = /\bfor kids\b|toys|nursery|rhymes|toddler|cocomelon|baby shark|peppa|pinkfong|little angel|kidz bop|play doh|unboxing|toy review|mukbang|asmr food|gameplay|walkthrough|diy craft|pacifier|storytime|ted talk/i;
        const NON_MUSIC_TI = /\b(unboxing|toy review|mukbang|recipe|cooking with|gameplay|walkthrough|tutorial|diy craft|politics|podcast|prank|documentary|compilation|interview|behind the scenes|full movie|audiobook|meditation|white noise|timer|countdown|sound effect)\b/i;

        for (const e of result) {
          if (!e.id || seen.has(e.id)) continue;
          if (!e.duration || e.duration < 60 || e.duration > 600) continue;
          if (NON_MUSIC_CH.test(e.channel || '') || NON_MUSIC_TI.test(e.title || '')) continue;
          seen.add(e.id);
          allResults.push({
            videoId:   e.id,
            title:     e.title,
...
async function fetchAndLoadLyrics(track) {
  ...
  try {
    ...
    if (r.data?.length && r.data[0].syncedLyrics) {
      ...
      // Auto-sync: detect vocal onset in background
...
async function autoSyncLyrics(track, lrc) {
  try {
    ...
    // If the video has a silent intro, lyrics need delaying by that amount
    if (audioStart > 0.5) {
...
server.listen(PORT, '0.0.0.0', async () => {
  ...
  console.log(`    Platform:   ${config.platform?.type || 'pc'} (max ${config.platform?.maxHeight || 2160}p)`);
