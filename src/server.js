...
app.get('/api/stream/:videoId', async (req, res) => {
  ...
  // Step 1: get the direct CDN URL from yt-dlp (~2s, no downloading)
  let cdnUrl = '';
  try {
    cdnUrl = await new Promise((resolve, reject) => {
      ...
      proc.stderr.on('data', () => {});
      proc.on('close', code => code === 0 && out.trim() ? resolve(out.trim().split('\n')[0]) : reject());
  ...
  // Step 2: proxy the CDN URL, forwarding range requests so browser can seek
  const isRange = req.headers.range;
  const headers = {
    'User-Agent': 'Mozilla/5.0 (compatible)',
    'Referer': 'https://www.youtube.com/',
  };
  if (isRange) headers['Range'] = req.headers.range;

  const https = require('https');
  const http2 = require('http');
  const lib = cdnUrl.startsWith('https') ? https : http2;

  const proxyReq = lib.get(cdnUrl, { headers }, proxyRes => {
    // Forward status and all headers from YouTube CDN
